...
const handleAddProduct = async (e) => {
  ...
  try {
    ...
    console.log("Product1 added:", response.data);
    navigate("/diomondadmin"); // or another route if needed
  } catch (error) {
    console.error("Error adding product1:", error.response?.data || error);
  }
};
...
const deleteProduct = async (productId) => {
  try {
    ...
    console.log("Error deleting product1:", error);
  }
};
  ...
  return (
    <div>
      ...
      <h2 style={{ color: "#28a745",marginLeft:"10px" }}>Add New Diamond</h2>
      <form
            ...
            {products.length > 0 ? (
              products.map((product, index) => (
                <tr key={product._id}>
                  ...
                  <td>
                    <Link to={`/update/product1/${product._id}`} className="btn btn-info btn-sm">
